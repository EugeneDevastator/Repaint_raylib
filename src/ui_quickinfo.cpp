...
void QuickInfo_Draw(ImDrawList* dl, int gcx, int gcy, int gizR, AppState* state) {
    // ── Info text (base slider values, never modulated) ───────────────
    char buf[96];
    float baseRad = BParam_GetValue(&bpSize);
    float baseHd  = BParam_GetValue(&bpHardness);
    float baseOp  = BParam_GetValue(&bpOpacity);
    float baseCrv = BParam_GetValue(&bpCurvature);
    snprintf(buf, sizeof(buf), "Sz:%.0f Hd:%.0f%% Op:%.0f%% Crv:%.0f%% Sp:%.2f Sc:%.2f",
        baseRad, baseHd * 100, baseOp * 100, baseCrv * 100,
