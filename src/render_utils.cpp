...
unsigned int CreateTexRGBA16(int w, int h) {
    ...
    glBindTexture(GL_TEXTURE_2D, 0);   // ← this
    return id;
}
...
RenderTexture2D Load16BitRT(int w, int h) {
    ...
    if (t.id > 0) {
        ...
        t.depth.width = w; t.depth.height = h;
        t.depth.format = 19; t.depth.mipmaps = 1;
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, 0);  // sync GL state with raylib tracking
    }
    return t;
}